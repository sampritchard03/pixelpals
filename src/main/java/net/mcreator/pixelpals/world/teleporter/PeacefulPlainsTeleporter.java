...
public class PeacefulPlainsTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(PixelPals01ModBlocks.PEACEFUL_PLAINS_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("pixel_pals_01:peaceful_plains_portal"), poiType);
	...
	public PeacefulPlainsTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = PixelPals01ModBlocks.PEACEFUL_PLAINS_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
